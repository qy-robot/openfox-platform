...
func getTokenCacheKey(key string) string {
	return fmt.Sprintf("billing:cny:v1:token:%s", common.GenerateHMAC(key))
}

func getTokenCacheFenceKey(key string) string {
	return fmt.Sprintf("billing:cny:v1:token:fence:%s", common.GenerateHMAC(key))
}
...
redis.call('HSET', KEYS[1],
  ...
  'FundingMode', ARGV[17], 'TeamId', ARGV[18], 'DesktopSessionID', ARGV[19])
redis.call('EXPIRE', KEYS[1], ARGV[20])
return 1`
	...
	return common.RDB.Eval(context.Background(), script, []string{
		...
		token.FundingMode, token.TeamId, token.DesktopSessionID,
