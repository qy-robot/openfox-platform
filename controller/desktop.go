...
func CreateDesktopRelayToken(c *gin.Context) {
	...
	err = model.DB.Where("user_id = ? AND desktop_session_id = ? AND funding_mode = ? AND team_id = ? AND status = ? AND expired_time > ?", identity.UserID, identity.SessionID, input.FundingMode, input.TeamID, common.TokenStatusEnabled, now).Order("id desc").First(&token).Error
	...
	} else if token.ExpiredTime <= now+300 {
		// The relay credential is subordinate to the live desktop session, which
		// is validated on every model request. Extend this session-scoped row
		// instead of accumulating a new API-token record every hour.
		token.ExpiredTime = now + 3600
...
func DeleteDesktopSession(c *gin.Context) {
	...
	if err := model.DisableDesktopSessionTokens(identity.UserID, identity.SessionID); err != nil {
		common.SysError("failed to disable desktop relay credentials after logout: " + err.Error())
	}
