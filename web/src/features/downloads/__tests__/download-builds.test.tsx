...
test('a published release renders its multiline changelog', () => {
  render(
    <ReleaseChangelog
      manifest={{
        ...releasedManifest,
  ...
  expect(screen.getByText(/新增桌面安装包/)).toHaveTextContent(
    '新增桌面安装包 修复升级流程'
  )
})

test('a missing changelog or refresh error renders no release notes', () => {
  const { rerender } = render(
    <ReleaseChangelog manifest={releasedManifest} error={null} />
  )
  expect(screen.queryByRole('heading', { name: 'Changelog' })).toBeNull()

  rerender(
    <ReleaseChangelog
      manifest={{ ...releasedManifest, changelog: 'stale notes' }}
